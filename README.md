...
| [aws_iam_role_policy.account_access](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
...
| <a name="input_vpc_endpoint_enabled"></a> [vpc\_endpoint\_enabled](#input\_vpc\_endpoint\_enabled) | Set to true to create a VPC endpoint for Amazon Managed Prometheus. Requires vpc\_id to be set. | `bool` | `false` | no |
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | If `var.vpc_endpoint_enabled` is true, the ID of the VPC in which the endpoint will be used. | `string` | `""` | no |
